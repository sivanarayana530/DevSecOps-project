pipeline {
    ...
    stages {
        ...
        stage('Image Scan') {
            steps {
      	        sh 'docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v $(pwd):/workspace aquasec/trivy:latest image --format html -o /workspace/report.html praveensirvi/sprint-boot-app:latest'
            }
        }
